    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
  ...
  // Joint orientation is always stored in XYZ order; the animated Euler rotation uses the joint's rotate order.
  for (int i = 0; i < (int)localTransforms.size(); i++)
  {
    Mat3d jointOrientRotation;
    euler2Rotation(jointOrientationEulerAngles[i].data(), jointOrientRotation.data(), XYZ);

    Mat3d animatedRotation;
    euler2Rotation(eulerAngles[i].data(), animatedRotation.data(), rotateOrders[i]);

    Mat3d localRotation = jointOrientRotation * animatedRotation;
    localTransforms[i] = RigidTransform4d(localRotation, translations[i]);
  }

  // Propagate transforms from root to leaves using the update order.
  for (int jointID : jointUpdateOrder)
  {
    ...
    if (parentID < 0)
      ...
      globalTransforms[jointID] = globalTransforms[parentID] * localTransforms[jointID];
  }
}
