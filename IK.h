...
public:
  ...
  // Get/set damping parameter for regularization
  double getDamping() const { return dampingFactor; }
  void setDamping(double newDamping) { dampingFactor = newDamping; }
...
protected:
  ...
  double dampingFactor = 1e-3; // Tikhonov regularization damping (lower = faster but less stable, higher = more stable but less responsive)
