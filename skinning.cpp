...
void Skinning::applySkinning(const RigidTransform4d * jointSkinTransforms, double * newMeshVertexPositions) const
{
  for(int i = 0; i < numMeshVertices; i++)
  {
      // Always skin from the rest mesh; newMeshVertexPositions is output-only.
      Vec3d p(restMeshVertexPositions[3 * i], restMeshVertexPositions[3 * i + 1], restMeshVertexPositions[3 * i + 2]);
      Vec3d newP(0, 0, 0);
      for (int j = 0; j < numJointsInfluencingEachVertex; j++)
      {
          double weight = meshSkinningWeights[i * numJointsInfluencingEachVertex + j];
          int jointIndex = meshSkinningJoints[i * numJointsInfluencingEachVertex + j];
          newP += jointSkinTransforms[jointIndex].transformPoint(p) * weight;
      }
      newMeshVertexPositions[3 * i + 0] = newP[0];
      newMeshVertexPositions[3 * i + 1] = newP[1];
      newMeshVertexPositions[3 * i + 2] = newP[2];
  }
}
